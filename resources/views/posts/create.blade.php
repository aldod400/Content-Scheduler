        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="md:flex md:items-center md:justify-between mb-6">
                <div class="flex-1 min-w-0">
                    <h2 class="text-2xl font-semibold text-gray-900">{{ __('message.create_new_post') }}</h2>
                </div>
            ...
            <div class="bg-white shadow rounded-lg">
                <form action="{{ route('web.posts.store') }}" method="POST" class="space-y-6 p-6" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="status" id="status"                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                            ...
                            <option value="draft" {{ old('status', 'draft') === 'draft' ? 'selected' : '' }}>{{ __('message.draft') }}</option>
                            <option value="scheduled" {{ old('status') === 'scheduled' ? 'selected' : '' }}>{{ __('message.scheduled') }}</option>
                            <option value="published" {{ old('status') === 'published' ? 'selected' : '' }}>{{ __('message.published') }}</option>
                        </select>
                        @error('status')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div id="scheduledTimeContainer" class="hidden">
                        <label for="scheduled_time" class="block text-sm font-medium text-gray-700">
                            {{ __('message.schedule_time') }}
                        </label>                        <input type="datetime-local" name="scheduled_time" id="scheduled_time"
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    previewContainer.innerHTML = `
                        <div class="relative">
                            <img src="${e.target.result}" class="mt-2 h-32 w-auto rounded" alt="Preview">
